...
import logging
from tabulate import tabulate  # 优化：引入 tabulate 库
...
# 优化：降低 llama_parse 的 HTTP 请求日志级别
...
def analyze_pdf_complexity(
    ...
    try:
        with pdfplumber.open(file_path) as pdf:
            ...
            page_score = min(num_pages / 20, 1) * 15
            image_score = min(image_count / (num_pages * 3), 1) * 15
            continuity_score = (
            ...
            logger.info(
                f"文件 {file_path.name} 复杂性分析: score={total_score:.2f}, continuous={is_continuous}, visually_dense={is_visually_dense}"
            )
...
def get_parser_config(
    ...
    return LlamaParse(
        ...
        num_workers=4,
        max_timeout=7200,
        check_interval=10,  # 优化：从 5 改为 10
        verbose=True,
        do_not_cache=True,
    )
...
async def parse_files(
    ...
    for file_path in file_paths:
        try:
            ...
            if parse_success:
                ...
                file_docs_map[str(file_path)] = docs  # 优化：记录文件对应的文档
    ...
    return all_docs, report, file_docs_map  # 优化：返回 file_docs_map


async def main():
    start_time = datetime.datetime.now()
    logger.info(f"解析开始于 {start_time}")
    ...
    if input_path.is_file():
        ...
        try:
            ...
            docs = []
            report = {"files": [], "total_time": 0, "failures": []}
            file_docs_map = {}  # 优化：初始化 file_docs_map
            parse_success = False
            fallback_mode = (
                "parse_page_with_llm"
                if parser.parse_mode != "parse_page_with_llm"
            ...
            file_docs_map[str(input_path)] = docs  # 优化：记录文件对应的文档
